...
namespace DorllyService.Common
{
    public class Utils
    {
        ...
        public static int StrToInt(string expression)
        {
            ...
            return Convert.ToInt32(StrToFloat(expression, 0));
        }
        /// <summary>
        /// Object型转换为decimal型
        /// </summary>
        ...
        public static bool SqlFilter(string word, string InText)
        {
            ...
            {
                if ((InText.ToLower().IndexOf(i + " ", StringComparison.Ordinal) > -1) || (InText.ToLower().IndexOf(" " + i) > -1))
                {
                    return true;
                }
            }
        ...
        //    //return GetMapPath(UtilConf.Configuration["Site:" + xmlName]);
        //}
        //#endregion
        ...
        {
            if (strPath.ToLower().StartsWith("http://", StringComparison.Ordinal))
            {
                return strPath;
            }
        ...
        public static void DeleteUpFile(string _filepath)
        {
            ...
            if (_filepath.LastIndexOf("/", StringComparison.Ordinal) >= 0)
            {
                string thumbnailpath = _filepath.Substring(0, _filepath.LastIndexOf("/", StringComparison.Ordinal)) + "mall_" + _filepath.Substring(_filepath.LastIndexOf("/") + 1);
                string fullTPATH = GetMapPath(thumbnailpath); //缩略图
        ...
        public static void DeleteContentPic(string content, string startstr)
        {
            ...
            foreach (Match math in m)
            {
                ...
                try
                {
                    if (imgUrl.ToLower().StartsWith(startstr.ToLower(), StringComparison.Ordinal) && File.Exists(fullPath))
                    {
                        File.Delete(fullPath);
                    }
                }
        ...
        public static string GetFileExt(string _filepath)
        {
            ...
            if (_filepath.LastIndexOf(".", StringComparison.Ordinal) > 0)
            {
                return _filepath.Substring(_filepath.LastIndexOf(".", StringComparison.Ordinal) + 1); //文件扩展名，不含“.”
            }
            return "";
        }
        ...
        {
            return _filepath.Substring(_filepath.LastIndexOf(@"/", StringComparison.Ordinal) + 1);
        }
        ...
        public static string CombUrlTxt(string _url, string _keys, params string[] _values)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(urlParams.ToString()) && _url.IndexOf("?", StringComparison.Ordinal) == -1)
                    urlParams.Insert(0, "?");
            }
            catch
            {
        ...
        public static string UrlExecute(string urlPath)
        {
            ...
            try
            {
                ...
                byte[] responseData = wc.UploadData(urlPath, "GET", postData);
        ...
        ///// <summary>
        ///// 当前根URL，中域信息之前的部分，不含页面名称以及参数等地址前加http://
        ///// 主要用于统计，不带http://也能正常访问，但为了避免不可预知问题还是加上
        ///// </summary>
        //public static string GetCurrentFullHost()
        //{
        //    return "http://" + JHRequest.GetCurrentFullHost();
        //}
        ///// <summary>
        ///// 当前根URL，中域信息之前的部分，不含页面名称以及参数等地址前加https://
        ///// </summary>
        //public static string GetCurrentFullHosts()
        //{
        //    return "https://" + JHRequest.GetCurrentFullHost();
        //}
    }
}
