...
namespace DorllyService.Service
{
    public class RepositoryBase<T> : IRepository<T> where T : class, new()
    {
        ...
        private readonly DbContext context = new DbConfig().db;
        /// <summary>
        ...
        /// 数据上下文--->拓展属性
        /// </summary>
        //public DbConfig Config => new DbConfig();
        /// <summary>
        /// 公用泛型处理属性
        /// 注:所有泛型操作的基础
        /// </summary>
        ...
        public virtual int SaveList<T1>(List<T1> t) where T1 : class
        {
            try
            {
                ...
                Context.Set<T1>().Local.Clear();
                foreach (T1 item in t)
                {
        ...
        public virtual int UpdateList<T1>(List<T1> t) where T1 : class
        {
            ...
            try
            {
                foreach (T1 item in t)
                {
                    Context.Entry<T1>(item).State = System.Data.Entity.EntityState.Modified;
                }
                return Context.SaveChanges();
            }
            catch (Exception e)
        ...
        {
            IQueryable<T> entry = dbSet.Where(predicate);
            return (entry.Any());
        }
        ...
        {
            IEnumerable result = Context.Database.SqlQuery(typeof(int), sql, para);
        ...
        public virtual DbQuery<T> LoadQueryAll(Expression<Func<T, bool>> predicate)
        {
            try
            {
                if (predicate != null)
                {
                    return dbSet.Where(predicate) as DbQuery<T>;
                }
                return dbSet;
            }
            catch (Exception e)
        ...
        public virtual IEnumerable<T> LoadEnumerableAll(string sql, params DbParameter[] para)
        {
            try
            {
                return Context.Database.SqlQuery<T>(sql, para);
            }
            catch (Exception e)
            {
                throw e;
        ...
        public virtual System.Collections.IEnumerable LoadEnumerable(string sql, params DbParameter[] para)
        {
            try
            {
                return Context.Database.SqlQueryForDynamic(sql, para);
            }
            catch (Exception e)
            {
                throw e;
        ...
        public virtual List<T> SelectBySql(string sql, params DbParameter[] para)
        {
            try
            {
                return Context.Database.SqlQuery(typeof(T), sql, para).Cast<T>().ToList();
            }
            catch (Exception e)
            {
                throw e;
        ...
        public virtual List<T1> SelectBySql<T1>(string sql, params DbParameter[] para)
        {
            try
            {
                return Context.Database.SqlQuery<T1>(sql, para).ToList();
            }
            catch (Exception e)
            {
                throw e;
        ...
        public virtual Common.PageInfo Query(int index, int pageSize, string tableName, string field, string filter, string orderby, string group, params DbParameter[] para)
        {
            ...
            //获取当前条件下数据总条数
            int count = Context.Database.SqlQuery(typeof(int), "select count(*) from (" + tableName + ") where " + filter, para).Cast<int>().FirstOrDefault();
            ...
            IEnumerable list = ExecuteSqlQuery(sql, para) as IEnumerable;
            if (list != null)
            {
        ...
        {
            return Query(index, pageSize, sql, null, null, orderby, null, para);
        }
        /// <summary>
        /// 多表联合分页算法
        /// </summary>
        ...
        {
            IEnumerable<object> enumerable = (query as System.Collections.IEnumerable).Cast<object>();
            if (index < 1)
            {
                index = 1;
            }
        ...
        public virtual object ExecuteSqlCommand(Dictionary<string, object> sqllist)
        {
            try
            {
                ...
                using (Transaction)
                {
                    ...
                    {
                        rows += Context.Database.ExecuteSqlCommand(enumerator.Current.Key, enumerator.Current.Value);
                    }
                    Commit();
                }
                return rows;
        ...
        DbSet<T> IRepository<T>.LoadQueryAll(Expression<Func<T, bool>> predicate)
        {
